...
const myersonian_1 = require("./myersonian");
...
console.log("-".repeat(80));
(0, myersonian_1.demonstrateMyersonianFramework)();
// TEST 2: Virtual value functions
console.log("\n[TEST 2] Virtual Value Function Tests");
console.log("-".repeat(80));
const testScores = [75, 80, 85, 90, 95];
const distribution = (0, myersonian_1.buildDistribution)([85, 90, 88, 92, 78, 95, 87, 91, 89, 93]);
console.log("Testing virtual value computations:");
for (const score of testScores) {
    const cdf = (0, myersonian_1.empiricalCDF)(distribution.historicalScores, score);
    const pdf = (0, myersonian_1.estimatePDF)(distribution.historicalScores, score);
    console.log(`  Score ${score}: CDF=${cdf.toFixed(4)}, PDF=${pdf.toFixed(4)}`);
}
// TEST 3: Optimal tier boundaries
console.log("\n[TEST 3] Optimal Tier Boundaries (Theorem 3.2)");
console.log("-".repeat(80));
const boundaries = (0, myersonian_1.computeOptimalTierBoundaries)(distribution);
...
for (const test of allocationTests) {
    const tier = (0, myersonian_1.allocateOptimalTier)(test.score, boundaries);
    const tierName = tier === "MARTYR" ? "MARTYR" : tier === "SOVEREIGN" ? "SOVEREIGN" : "REJECT";
...
for (const score of testScores) {
    const reward = (0, myersonian_1.calculateICReward)(score, distribution);
    const marginal = (0, myersonian_1.calculateMarginalICReward)(score, distribution);
    console.log(`  Score ${score}%: R(σ)=${reward.toFixed(2)}, dR/dσ=${marginal.toFixed(4)}`);
...
for (const test of spreadTests) {
    const spread = (0, myersonian_1.calculateOptimalCrisisSpread)(4.0, // $4 base price
    0.3, // 30% crisis volatility
    0.15, // 15% normal volatility
    test.advantage, test.riskAversion);
...
const slashResult = (0, myersonian_1.calculateSlashingAmount)(claimedScore, actualScore, distribution);
...
const posterior = (0, myersonian_1.updateMMCredibility)(prior, proofOutcome, weight);
